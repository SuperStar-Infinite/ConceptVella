...
router.get("/:id", async (req, res) => {
    ...
    // Get checkpoints
    const { data: checkpoints, error: checkpointsError } = await supabase_1.supabase
...
router.post("/", auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (checkpoints && checkpoints.length > 0) {
            ...
            const { error: checkpointsError } = await supabase_1.supabase
                .from("challenge_checkpoints")
        ...
        const { data: challengeWithCheckpoints } = await supabase_1.supabase
            .from("challenge_checkpoints")
            .select("*")
...
router.put("/:id", auth_1.requireAdmin, async (req, res) => {
    const challengeId = req.params.id;
    const { title, description, type, difficulty, region_code, status, starts_at, ends_at, } = req.body;
    ...
    const { data: challenge, error: updateError } = await supabase_1.supabase
        .from("challenges")
        .update(updates)
        .eq("id", challengeId)
        .select()
...
router.get("/:id/progress", auth_1.requireAuth, async (req, res) => {
    const challengeId = req.params.id;
    const userId = req.user.id;
    const { data: progress, error } = await supabase_1.supabase
        .from("user_challenge_progress")
        .select("*")
        .eq("challenge_id", challengeId)
        .eq("user_id", userId)
...
router.put("/:id/progress", auth_1.requireAuth, async (req, res) => {
    ...
    updates.updated_at = new Date().toISOString();
    const { data: progress, error: updateError } = await supabase_1.supabase
        .from("user_challenge_progress")
        .update(updates)
        .eq("id", existing.id)
        .select()
 ...
 */
router.post("/:id/checkpoints", auth_1.requireAdmin, async (req, res) => {
    const challengeId = req.params.id;
    const { poi_id, requirement_type = "visit", requirement_value, order_index } = req.body;
    // Check challenge exists
    const { data: challenge, error: checkError } = await supabase_1.supabase
        .from("challenges")
        .select("id")
        .eq("id", challengeId)
        .single();
    ...
    if (finalOrderIndex === undefined) {
        const { data: existingCheckpoints } = await supabase_1.supabase
            .from("challenge_checkpoints")
            .select("order_index")
            .eq("challenge_id", challengeId)
    ...
    const { data: checkpoint, error: insertError } = await supabase_1.supabase
        .from("challenge_checkpoints")
        .insert({
        challenge_id: challengeId,
        poi_id: poi_id || null,
 ...
 */
router.put("/:id/checkpoints/:checkpointId", auth_1.requireAdmin, async (req, res) => {
 ...
 */
router.delete("/:id/checkpoints/:checkpointId", auth_1.requireAdmin, async (req, res) => {
    const { checkpointId } = req.params;
    // Check checkpoint exists
    const { data: existing, error: checkError } = await supabase_1.supabase
        .from("challenge_checkpoints")
